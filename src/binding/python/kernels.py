...
@torch.library.custom_op("llmq::rmsnorm_backward", mutates_args=("dinp", "dweight", "scratch", "dresidual"))
...
@torch.library.custom_op("llmq::fused_residual_rmsnorm_forward", mutates_args=("residual", "normed", "rrms"))
...
# RoPE
@torch.library.custom_op("llmq::rope_forward", mutates_args=("out",))
...
@torch.library.custom_op("llmq::rope_backward", mutates_args=("dinp",))
...
# SwiGLU
@torch.library.custom_op("llmq::swiglu_forward", mutates_args=("out",))
...
@torch.library.custom_op("llmq::swiglu_backward", mutates_args=("dinp",))
...
# Attention (cuDNN)
@torch.library.custom_op("llmq::attention_forward", mutates_args=("out", "stats"))
...
# Matmul and bias
@torch.library.custom_op("llmq::matmul", mutates_args=("c",))
...
@torch.library.custom_op("llmq::backward_bias", mutates_args=("dbias",))
...
@torch.library.custom_op("llmq::quantize_with_abs_max", mutates_args=("out",))
...
@torch.library.custom_op("llmq::quantize_and_transpose_with_abs_max", mutates_args=("out",))
...
def vector_add_sr(dest: torch.Tensor, left: torch.Tensor, right: torch.Tensor, scale: torch.Tensor, seed: int, stream: int = 0) -> None:
...
# Encoder backward
@torch.library.custom_op("llmq::encoder_backward", mutates_args=("dwte", "scratch"))
