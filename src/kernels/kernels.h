...
void qk_norm_forward(nv_bfloat16* out, float* r_rms, const nv_bfloat16* inp,
                     ...
                     float epsilon, int BT, int Nq, int Nkv, int HeadDim,
                     cudaStream_t stream);
void qk_norm_forward(Tensor& out, Tensor& r_rms, const Tensor& inp,
...
std::size_t qk_norm_and_rope_backward_scratch_size(int Nq, int Nkv, int HeadDim, ETensorDType dtype, const cudaDeviceProp& dp);
