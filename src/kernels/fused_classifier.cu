...
template <class floatX, bool WriteDLogits>
__global__ void __launch_bounds__(1024, 1)
    ...
    if(threadIdx.x == 0) {
        losses[idx] -= (float)logits[idx * P + ix] - lse;
        lse_out[idx] = lse;
    }
    z_reg = z_reg * lse;
...
void fused_classifier_imp(Type* logits, float* losses, float* lse,
    ...
    if(write_dlogits) {
        fused_classifier_kernel5<<<grid_size, block_size, 0, stream>>>(logits, losses, lse, dloss, targets,
                                                                       z_loss, V, P, std::bool_constant<true>());
    } else {
        fused_classifier_kernel5<<<grid_size, block_size, 0, stream>>>(logits, losses, lse, dloss, targets,
                                                                       z_loss, V, P, std::bool_constant<false>());
    }
 ...
 *
 * This kernel assumes a block size of 32.
 */
template<class floatX>
__global__ void grouped_loss_sum_kernel(float* out, const floatX* data, int B, int T) {
    constexpr int VecSize = 16 / sizeof(floatX);
